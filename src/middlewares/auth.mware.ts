...
import { TCombineOptValidate } from "@globals/validation-route.global";
...
export default class ValidateRequestBodyMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    try {
      const validate = (req.validations as TCombineOptValidate)

      if(typeof validate.auth === 'boolean' && validate.auth || typeof validate.auth !== 'boolean') {
        const [_bearer, _token]: string[] = req.headers['authorization'].split(' ')
      ...
      res.asJson(HttpStatus.UNAUTHORIZED, { message: '[AUTH] Authentication failed.' })
    }
