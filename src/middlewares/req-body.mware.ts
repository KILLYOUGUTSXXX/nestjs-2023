...
export default class ValidateRequestBodyMiddleware implements NestMiddleware {
  use(req: Request, res: Response, next: NextFunction) {
    const registryTemplate = req.validations as TCombineOptValidate
    try {
      if(typeof registryTemplate.maxBodySize === 'number' && registryTemplate.maxBodySize > 0) {
        const incomingSize = +((new TextEncoder().encode(JSON.stringify((req.body || {}))).length / 1024.0 / 1024.0).toFixed(5))
