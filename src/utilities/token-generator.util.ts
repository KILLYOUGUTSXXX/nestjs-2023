...
const secureToken = function (token: string, type: 'dec' | 'enc'): string {
  ...
  if(splitToken.length === 3) {
    if(type === 'dec') {
      const decrypted = CryptoJS.AES.decrypt(splitToken[1], process.env.AFX_PAYLOAD_REENCRYPT_KEY)
      splitToken[1] = decrypted.toString(CryptoJS.enc.Utf8)
    } else if (type === 'enc') {
      const encrypted = CryptoJS.AES.encrypt(splitToken[1], process.env.AFX_PAYLOAD_REENCRYPT_KEY)
...
export interface IJwtCustom {
  readonly generateToken: (payload: Object) => Promise<string>,
  ...
  return <T extends { new (...args: any[]): {} }> (
    ...
    return class extends constructor {
      ...
      constructor(...agg: any[]) {
        ...
        if(!this.jwtService) {
          throw new Error(`JWT Services is never called in class "${constructor.name}".`)
        }
